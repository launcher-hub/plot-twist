...
/**
 * Update Tab (DOM Cache)
 * 
 * NOTE: These must be declared before any early call paths
 * can invoke `prepareSettings()` while this script is still evaluating.
 */
...
// Bind the add mojang account button (guarded — element may be removed by generator).
const _guardedSettingsAddMojang = document.getElementById('settingsAddMojangAccount')
if(_guardedSettingsAddMojang) _guardedSettingsAddMojang.onclick = (e) => {
...
const _settingsAddMicrosoftAccount = document.getElementById('settingsAddMicrosoftAccount')
if(_settingsAddMicrosoftAccount){
    _settingsAddMicrosoftAccount.onclick = (e) => {
...
async function populateJavaExecDetails(execPath){
    if(!execPath || !execPath.trim()){
        if(typeof settingsJavaExecDetails !== 'undefined' && settingsJavaExecDetails){
    ...
    try {
        details = await Promise.race([
            validateSelectedJvm(ensureJavaDirIsRoot(execPath), server.effectiveJavaOptions.supported),
